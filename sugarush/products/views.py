...
def show_shop(request,id=None):
 context = {'shop_list' : Shops.objects.all().order_by('shop_name'),'company':Shops.objects.filter(id = id),'prod_list':Products.objects.filter(shop_id = Shops.objects.get(id = id))}
